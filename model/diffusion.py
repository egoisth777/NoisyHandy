...
from utils.utils import default, identity, normalize_to_neg_one_to_one, unnormalize_to_zero_to_one, count_parameters
...
def load_diffusion_model(config, device='cpu'):
    ...
    checkpoint = [x for x in os.listdir(os.path.join(config.out_dir, config.exp_name))][0]
